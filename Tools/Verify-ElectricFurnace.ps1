...
if (!$OutputDirectory) { $OutputDirectory=Join-Path $project 'Logs\ElectricFurnaceVerification' }
...
if ($Build) {
    ...
    Set-Content (Join-Path $project 'Logs\build-request.txt') 'electric-furnace-build' -NoNewline
...
$player=Join-Path $project 'Builds\ElectricFurnace\RivetReach.exe'
$process=Start-Process -FilePath $player -ArgumentList @('-screen-fullscreen','0','-screen-width','1280','-screen-height','800','-rr-verify','-rr-electric-furnace-review','-rr-output',('"'+$OutputDirectory+'"'),'-logFile',('"'+$OutputDirectory+'\player.log"')) -PassThru
if (!$process.WaitForExit(300000)) { throw 'Electric furnace review timed out; process preserved for diagnosis.' }
