...
if (!$OutputDirectory) { $OutputDirectory=Join-Path $project ('Logs\Beds-'+[DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss')) }
...
if ($Build) {
    ...
    Set-Content $request 'bed-build' -NoNewline
...
$player=Join-Path $project 'Builds\Beds\RivetReach.exe'
$arguments=@('-screen-fullscreen','0','-screen-width','1280','-screen-height','800','-rr-verify','-rr-bed-review','-rr-output',('"'+$OutputDirectory+'"'),'-logFile',('"'+$OutputDirectory+'\player.log"'))
$process=Start-Process -FilePath $player -ArgumentList $arguments -PassThru
if (!$process.WaitForExit(300000)) { throw 'Beds review timed out; player preserved for diagnosis.' }
