param([switch]$Build,[switch]$Baseline,[string]$OutputDirectory,[int]$TimeoutSeconds=900)
...
$arguments=@('-screen-fullscreen','0','-screen-width','1280','-screen-height','720','-rr-verify','-rr-shadow-review','-rr-output',('"'+$OutputDirectory+'"'),'-logFile',('"'+(Join-Path $OutputDirectory 'player.log')+'"'))
...
$shadow=Get-Content (Join-Path $OutputDirectory 'shadow-report.json') -Raw | ConvertFrom-Json
...
Write-Output "$($shadow.result): $($shadow.aoMethod); frozen channel delta $($shadow.samples[0].meanChannelDelta). $OutputDirectory"
