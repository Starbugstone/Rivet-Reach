param([string]$OutputDirectory,[string]$SaveDirectory,[switch]$LegacyRenewables,[switch]$FinalBuild,[switch]$KitchenOnly,[switch]$Hud,[switch]$ToolsBuild)
...
$player=Join-Path $project $(if ($ToolsBuild) {'Builds\Tools\RivetReach.exe'} elseif ($FinalBuild) {'Builds\FoodBalanceFinal\RivetReach.exe'} else {'Builds\FoodBalance\RivetReach.exe'})
...
if ($Hud) { $scenario='-rr-food-hud-review' }
