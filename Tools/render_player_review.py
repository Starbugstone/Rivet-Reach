...
scene=bpy.context.scene;scene.render.engine='CYCLES';scene.cycles.samples=24;scene.cycles.use_denoising=True;scene.render.resolution_x=1400;scene.render.resolution_y=1200;scene.render.resolution_percentage=100
scene.view_settings.view_transform='AgX';scene.render.image_settings.file_format='PNG'
scene.render.filepath=str(root/'Logs/player-review-front.png');bpy.ops.render.render(write_still=True)
o.location=(.55,6,2.35);o.rotation_euler=(Vector((0,0,.96))-o.location).to_track_quat('-Z','Y').to_euler()
scene.render.filepath=str(root/'Logs/player-review-back.png');bpy.ops.render.render(write_still=True)
