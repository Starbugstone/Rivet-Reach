...
for name,x in [('ExplorerMale',-.48),('ExplorerFemale',.48)]:
    ...
    for image in bpy.data.images:
        if image.source=='FILE':image.filepath=str(root/'Assets/RivetReach/Resources/Characters'/Path(image.filepath.replace('\\','/')).name)
...
if '--eevee' in sys.argv:
    scene.render.engine=next(e.identifier for e in scene.render.bl_rna.properties['engine'].enum_items if 'EEVEE' in e.identifier)
scene.render.filepath=str(root/'Logs/player-v4-front.png');bpy.ops.render.render(write_still=True)
if '--front-only' in sys.argv:sys.exit(0)
