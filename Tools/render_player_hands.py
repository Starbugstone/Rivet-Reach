...
for action,frame,label in [('FP_Idle',1,'idle'),('FP_Mine',8,'strike')]:
 rig.animation_data.action=bpy.data.actions[action];scene.frame_set(frame)
 scene.render.filepath=str(root/('Logs/player-v4-hands-'+label+'.png'));bpy.ops.render.render(write_still=True)
