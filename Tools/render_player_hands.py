...
allowed=[g.index for g in model.vertex_groups if g.name.endswith('R') and any(n in g.name for n in ['Arm','Forearm','Hand','Finger','Thumb'])]
remove=[v.index for v in model.data.vertices if sum(g.weight for g in v.groups if g.group in allowed)<.5]
...
for action,frame,label in [('FP_Idle',1,'idle'),('FP_Mine',3,'lift'),('FP_Mine',5,'sweep'),('FP_Mine',7,'follow-through'),('FP_Mine',9,'return')]:
 rig.animation_data.action=bpy.data.actions[action];scene.frame_set(frame)
 scene.render.filepath=str(root/('Logs/interaction-blender-'+label+'.png'));bpy.ops.render.render(write_still=True)
