...
if (!$OutputDirectory) { $OutputDirectory = Join-Path $project 'Logs\IndustryVerification' }
...
if ($Build) {
    ...
    Set-Content -Encoding ascii $request 'industry-build'
...
$executable = Join-Path $project 'Builds\Industry\RivetReach.exe'
if (!(Test-Path $executable)) { throw 'Use -Build with the pinned Editor open.' }
$arguments = @('-screen-fullscreen','0','-screen-width','1280','-screen-height','720','-rr-verify','-rr-industry-review','-rr-output',('"'+$OutputDirectory+'"'),'-logFile',('"'+(Join-Path $OutputDirectory 'player.log')+'"'))
$process = Start-Process -FilePath $executable -ArgumentList $arguments -PassThru
if (!$process.WaitForExit(900000)) { throw 'Industry review exceeded fifteen minutes. Player remains available for diagnosis.' }
$report = Join-Path $OutputDirectory 'runtime-report.json'
if (!(Test-Path $report)) { throw 'No runtime report; inspect player.log.' }
Get-Content $report
if ($process.ExitCode -ne 0 -or (Get-Content -Raw $report | ConvertFrom-Json).result -ne 'PASS') { throw 'Industry verification failed.' }
