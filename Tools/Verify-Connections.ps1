...
if (!$Executable) { $Executable=Join-Path $project 'Builds\Connections\RivetReach.exe' }
if (!$OutputDirectory) { $OutputDirectory=Join-Path $project ('Logs\Connections\Runtime-'+[DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss')) }
...
$scenario=if ($LegacyDirectory) { '-rr-connections-legacy-review' } else { '-rr-connections-review' }
...
if (!$process.WaitForExit(360000)) { throw 'Connections verification exceeded six minutes; player retained for diagnosis.' }
