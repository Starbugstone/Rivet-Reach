...
## Required crafting progression by material tier

The user requires rarer and higher-tier ores/materials in the construction recipes of more powerful and higher-tier equipment. Gold and diamond must have meaningful advanced crafting uses; **Floater Rock is also a high-tier crafting material**, earned from Floater encounters. Apply this rule when adding or revising advanced recipes, including through consumed high-tier components. Keep the basic mining, processing and workshop bootstrap obtainable before the upgrades it enables; avoid circular resource dependencies. [ECONOMY.md](.docs/ECONOMY.md#material-tiered-construction--2026-09-13) owns the progression rationale. The current bridge-pair gold/diamond costs and ranged-pump gold upgrade are specified in [BRIDGES.md](.docs/BRIDGES.md#crafting) and [RANGED_PUMP.md](.docs/RANGED_PUMP.md#working-rules). Recipe rebalancing must preserve existing saves with explicit compatibility for known previous definitions, retaining unrelated content checks.
...
## Authorized renewable electricity

The user authorized [Solar Panels and Wind Turbines](.docs/RENEWABLES.md) for issue #10. Solar must rise gradually from dawn to its midday maximum and decrease gradually to zero at night. Wind has a small baseline and smooth seeded random gusts, stronger in rain and maximum in storms. Turbines stop spinning when they deliver no power; battery charging counts as demand. Combine generation per connected cable grid, serve machines first and share surplus equally among eligible batteries, redistributing full-cell shares. Battery output is only for machine demand and must never charge another battery or feed a charging loop. Preserve separate cable grids, exact storage conservation, saved-seed/tick gust continuity, no offline or sleep-skipped production, cached open-sky gates and exact pre-renewable schema-16 compatibility. Keep advanced gold/diamond recipes, the illustrated wiki and issue checklist current; [verification](.docs/verification/RENEWABLE_RESULTS.md) distinguishes evidence from remaining balance review.
...
## Required capability interfaces and release review

The user requires interfaces as the project standard for gameplay capabilities and reusable behavior. Follow [CAPABILITIES.md](.docs/CAPABILITIES.md): an item or system may expose multiple small interfaces; prefer composition over combination-specific inheritance or repeated concrete-type/tag checks. Implement capability discovery and shared request/transfer rules centrally. Do not mark every object as supporting an optional interface whose operations it cannot perform.

Resolve and cache item capabilities when content loads. Runtime capability queries and recipe matching must avoid reflection, string-tag scans and per-tick allocations. Existing serialized tags and recipe/search labels remain compatibility inputs while runtime gameplay uses typed interfaces; preserve exact recipes, item identities, quantities and all current/legacy save checks during migration. New gameplay capabilities require explicit interfaces and registered mappings. Known recipe labels and typed selectors must share one compiled membership authority; register inherited interfaces explicitly and never infer new recipe memberships. Recompile dependent catalogs when replacing a registry snapshot. Prove fair allocation, face restrictions, transaction conservation and rebuild/dormancy behavior with shared contract tests and actual native stress measurements.

The authorized pre-0.1.0 pass covers speed, frame pacing, stability, code duplication/maintainability, weather reuse/caching and Minecraft-familiar interactions. Exercise a large working factory with all machine/storage/power types, farms and multiple mobs. Fix confirmed defects, preserve gameplay rules, and capture dramatic actual in-game factory/stress screenshots for the user's blog. Clearly distinguish scripted fixture captures and measured results from ordinary-play or release certification.

## Player responsibility for factory chunk loading

The user explicitly requires [factory residency](.docs/GAMEPLAY.md#factory-residency-and-player-responsibility) as a gameplay mechanic: players must provide [chunk loader coverage](.docs/BRIDGES.md#chunk-loaders) to keep factories operating while away. Unloaded machines and endpoints disconnect and retain exact state without production, resource transfer or catch-up on return. The engine must not compensate with implicit factory tickets, automatically loaded network paths, reduced-rate background production, simulated offline output or elapsed-time credits. Loaded portions may operate only through fully resident valid routes; the player must cover machines, storage, all required pipe/cable chunks, bridge endpoints and extraction targets. Preserve existing explicit loader tickets, normal player residency, pause/save rules and resource conservation. Optimize scheduling and representation rather than weakening this mechanic.

The user’s 0.1.0 gameplay performance target is **above 60 FPS normally**, with **45 FPS as the worst-case floor** (16.67 ms and 22.22 ms per frame respectively). Include busy factories, weather, farms/mobs and chunk transitions in measurements. Report frame-time distributions and every floor breach; averages or a smooth stationary scene do not establish this requirement. Identify hardware, resolution, settings and build, and report loading/save stalls separately rather than hiding gameplay stalls.
