...
The project is in early development. The user authorized and the repository now implements the first terrain/FPS/inventory POC in Unity 6000.4.4f1 / URP. [FIRST_POC.md](.docs/FIRST_POC.md) owns its run/build instructions and implementation record; [verification results](.docs/verification/FIRST_POC_RESULTS.md) distinguish measured evidence from remaining review. Keep work scoped to the current request. This is not authorization to implement the full roadmap, upgrade the Editor or install live integrations. Do not describe planned full-game systems as implemented.

The user has locked the first playable milestone in documentation: terrain-only world generation and chunk streaming, FPS movement, a 3D player, fist mining, functional inventory and a nonfunctional crafting placeholder inside inventory. No generated structures. [DEVELOPMENT_STRATEGY.md](.docs/DEVELOPMENT_STRATEGY.md#6-locked-first-step-and-provisional-later-sequence) owns its detailed scope and review gate. The user subsequently authorized implementation of this first POC. Review this slice with the user before selecting subsequent work; the later roadmap groups are candidates, not an automatic queue.

Player appearance must offer male and female model choices, both supporting changeable skins. Keep appearance selection separate from gameplay dimensions/capabilities and verify actual model/rendering cost before claiming performance. Treat the illustrated character as one proposed appearance, not a fixed identity. [GAMEPLAY.md](.docs/GAMEPLAY.md#15-player-skins) owns skin behaviour and the working first-step minimum; [CONTENT_PIPELINE.md](.docs/CONTENT_PIPELINE.md#6-player-skin-authoring-contract) owns model/texture constraints. The first slice includes both model variants and two local test skins; further customization remains scoped by later requests.
