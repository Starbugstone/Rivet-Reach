    ...
    public static class DoorChecks
    {
        ...
        public static void Run()
        {
            ...
            try
            {
                ...
                {
                    oldItems.items=items.items.Where(i=>i.runtimeId!=IndustryId.WoodenDoor&&(crank||i.runtimeId!=IndustryId.HandCrank)).Select(i=>JsonUtility.FromJson<ItemDefinition>(JsonUtility.ToJson(i))).ToArray();
                    catalog.recipes=originals.Where(r=>r.stableId!="rivet:wooden_door"&&(crank||r.stableId!="rivet:industry_170")).ToArray();
                    ...
                    var bytes=new SaveStore("unused",oldItems).Encode(entry,w=>w.Write(314));using(var reader=store.Open(bytes,out _))Check(reader.ReadInt32()==314,"Pre-door content loads, crank present="+crank);
                    oldItems.items[0].attackDamage++;bytes=new SaveStore("unused",oldItems).Encode(entry,w=>w.Write(314));bool rejected=false;
