...
namespace RivetReach.Editor
{
    ...
    public static class TerrainTiles
    {
        public static Texture2DArray Build()
        {
            ...
            // Use the orthographic face of the authored Azure mesh, also used for its icon.
            var azure=new Texture2D(size,size,TextureFormat.RGBA32,false);
            try
            {
                if(!azure.LoadImage(System.IO.File.ReadAllBytes("Assets/RivetReach/Resources/Industry/AzureOreTile.png"))||azure.width!=size||azure.height!=size)
                    throw new System.InvalidOperationException("Azure ore requires its authored 64×64 face render.");
