...
namespace RivetReach.Editor
{
    ...
    public static class StarterRecipeChecks
    {
        public static int Run(ItemRegistry items,RecipeRegistry registry)
        {
            ...
            Recipe(IndustryId.WoodenDoor,3,BlockId.Planks,"MM/MM/MM");
            ...
            Check(verified.Count==56&&registry.Recipes.Count(r=>!r.Id.StartsWith("rivet:industry_")&&!r.Id.StartsWith("rivet:farm_")&&r.Id!="rivet:wrench"&&r.Id!="rivet:compost_bin"&&r.Id!="rivet:auto_composter"&&r.Id!="rivet:fishing_rod"&&r.Id!="rivet:bed"&&r.Id!="rivet:bulk_crate"&&r.Id!="rivet:crate_controller")==56,"The survival subset retains exactly the reviewed 56 recipes");
            ...
            string report=$"PASS: {checks} independent recipe acceptance checks; all 56 active layouts and output quantities.\nDots are empty cells; slashes separate rows.\n"+string.Join("\n",lines)+"\n";
