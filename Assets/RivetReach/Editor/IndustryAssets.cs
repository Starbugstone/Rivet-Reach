            ...
            foreach(string path in Directory.GetFiles("Assets/RivetReach/Resources/Industry","*.fbx"))
            {
                ...
                try
                {
                    foreach(var filter in source.GetComponentsInChildren<MeshFilter>())
                    {
                        ...
                        part.AddComponent<MeshFilter>().sharedMesh=mesh;part.AddComponent<MeshRenderer>().sharedMaterial=part.name=="StatusLight"?Resources.Load<Material>("Industry/Status"):material;
                    }
