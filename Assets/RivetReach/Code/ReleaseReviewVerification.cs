...
namespace RivetReach
{
    public sealed partial class RuntimeVerification
    {
        ...
        IEnumerator ReviewReleasePerformance()
        {
            ...
            csv.WriteLine("workload,frame,frame_ms,gc_bytes,gpu_ms,"+string.Join(",",names)+","+string.Join(",",Array.ConvertAll(names,n=>"alloc_"+n))+",machine_views_created,crate_views_created,timing_timestamp,cpu_main_ms,cpu_render_ms,present_wait_ms,draw_calls,batches,set_pass,srp_draw_calls,shadow_casters");
            ...
            IEnumerator Sample(string workload,int requestedCount)
            {
                ...
                for(int frame=0;frame<count;frame++)
                {
                    ...
                    for(int i=0;i<3;i++)csv.Write(","+cpuTimings[frame,i].ToString("F4",CultureInfo.InvariantCulture));for(int i=0;i<5;i++)csv.Write(","+renders[frame,i]);csv.WriteLine();
                }
            ...
            finally{RuntimeCosts.MeasureAllocations=false;foreach(var scope in scopes)scope.Dispose();foreach(var counter in renderCounters)counter.Dispose();}
        }
    }
}
