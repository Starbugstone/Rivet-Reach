...
namespace RivetReach
{
    ...
    {
        sealed class View {public GameObject Root;public MachineState Machine;public ulong Key=ulong.MaxValue;public Renderer Status;}
        readonly Dictionary<BlockPos,View> views=new Dictionary<BlockPos,View>();
        readonly Dictionary<ulong,Mesh[]> cache=new Dictionary<ulong,Mesh[]>();
        ...
        void Update()
        {
            ...
            foreach(var m in sim.EligibleMachines)
            {
                ...
                if(v.Status!=null)v.Status.SetPropertyBlock(properties);
            }
        ...
        {
            faces=0;joins=0;var c=m.Structure;bool formed=c?.Formed==true;
            for(int f=0;f<6;f++)
            {
                ...
                for(int t=0;t<6;t++)
                {
                    ...
                    if(c.Validation.Members.ContainsKey(adjacent)&&game.World.Get(adjacent)==m.Definition.Id&&!c.Bounds.Contains(IndustryDefinition.Neighbor(adjacent,wf)))joins|=1UL<<(f*6+t);
                }
            }
            // Exact 8-bit ID, 6-bit faces, and six 6-bit trim masks; no string/array per shell tick.
            return m.Definition.Id|((ulong)faces<<8)|(joins<<14);
        }
        Mesh[] Assemble(MachineState m,int faces,ulong joins)
        {
            ...
            foreach(var filter in prefab.GetComponentsInChildren<MeshFilter>())
            {
                ...
                if(name.StartsWith("Trim")){int face=name[4]-'0',side=name[6]-'0';if((faces&(1<<face))==0||(joins&(1UL<<(face*6+side)))!=0)continue;}
