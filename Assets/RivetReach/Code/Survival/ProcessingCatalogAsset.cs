...
namespace RivetReach
{
    ...
    public sealed class ProcessingCatalogAsset : ScriptableObject
    {
        ...
        public ProcessingRegistry Compile(ItemRegistry items)
        {
            ...
            for(int i=0;i<fuels.Length;i++){if(!items.HasTag(items.ResolveId(fuels[i].itemId),"burnable"))throw new ArgumentException("Fuel needs burnable tag: "+fuels[i].itemId);fuelSpecs[i]=new FuelSpec(fuels[i].itemId,fuels[i].ticks);}
