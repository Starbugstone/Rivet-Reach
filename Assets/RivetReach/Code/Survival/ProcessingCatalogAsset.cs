...
namespace RivetReach
{
    ...
    public sealed class ProcessingCatalogAsset : ScriptableObject
    {
        ...
        public ProcessingRegistry Compile(ItemRegistry items)
        {
            ...
            for(int i=0;i<fuels.Length;i++){if(items.Capability<IBurnable>(items.ResolveId(fuels[i].itemId))==null)throw new ArgumentException("Fuel needs IBurnable capability: "+fuels[i].itemId);fuelSpecs[i]=new FuelSpec(fuels[i].itemId,fuels[i].ticks);}
