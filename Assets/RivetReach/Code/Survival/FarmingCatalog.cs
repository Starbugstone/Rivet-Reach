...
namespace RivetReach
{
    [Serializable] public sealed class FarmingCatalog
    {
        ...
        public static void Load()
        {
            ...
            if(catalog==null)throw new ArgumentException("Invalid crop catalog.");catalog.Apply();
        }
        public void Apply()
        {
            var catalog=this;
            ...
            for(int i=0;i<catalog.crops.Length;i++)
            {
                ...
                if(c==null||c.key!=expected.key||c.first!=expected.first||c.stages!=expected.stages||c.planting!=expected.planting||c.produce!=expected.produce||c.stageTicks<20||c.stageTicks>24000||c.minYield<1||c.maxYield<c.minYield||c.maxYield>64||c.seedYield<0||c.seedYield>64)throw new ArgumentException("Invalid crop definition: "+c?.key);
            }
            for(int i=0;i<catalog.crops.Length;i++)
            {
                var c=catalog.crops[i];var expected=CropRules.Definitions[i];
