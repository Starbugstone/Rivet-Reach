...
namespace RivetReach
{
    public sealed partial class RuntimeVerification
    {
        IEnumerator ReviewIndustry()
        {
            ...
            {var p=origin.Offset(x,0,z);Check(world.Place(p,id),"Place "+game.Registry.Get(id).displayName);var m=sim.At(p);for(int i=0;i<rotation;i++)sim.Rotate(m);return m;}
            ...
            var chest=origin.Offset(7,0,-3);Check(world.Place(chest,BlockId.Chest),"Place output chest");
            Place(6,0,IndustryId.ItemPipe);
            var bench=origin.Offset(0,0,-3);world.Place(bench,IndustryId.Bench);world.Place(origin.Offset(1,0,-3),BlockId.Furnace);
            ...
            Aim(crusher);Check(game.TryInteractTarget(),"Interact action opens the unpowered machine");yield return Capture("industry-crusher-no-power-ui");game.SetMode(ScreenMode.Play);world.Place(breakCable,IndustryId.PowerCable);
            ...
            Check(game.Industry.Bucket(tank,true)&&tank.WaterMl==before+10000&&game.Inventory.Total(Fluids.EmptyBucket)==1,"Bucket transfer conserves 10 L and returns empty container");
            Check(game.Industry.Bucket(tank,false)&&tank.WaterMl==before&&game.Inventory.Total(Fluids.WaterBucket)==1,"Tank fill/empty round trip conserves water and bucket");game.SetMode(ScreenMode.Play);
