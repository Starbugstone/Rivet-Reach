...
namespace RivetReach
{
    ...
    {
        public static bool UsesModel(byte id)=>id==BlockId.Potato||id==BlockId.BakedPotato||id==BlockId.Apple;
        public static string Key(byte id)=>id==BlockId.Apple?"Apple":id==BlockId.BakedPotato?"BakedPotato":"Potato";
        public static Texture2D Icon(byte id)=>Resources.Load<Texture2D>(Folder(id)+Key(id)+"Icon");
        static string Folder(byte id)=>id==BlockId.Apple?"Orchard/":"Food/";
        ...
        public static GameObject Create(byte id,Transform parent,Material heldMaterial=null)
        {
            ...
            {
                renderer.sharedMaterial=heldMaterial!=null?heldMaterial:Resources.Load<Material>(id==BlockId.Apple?"Orchard/Apple":"Food/Potato");
