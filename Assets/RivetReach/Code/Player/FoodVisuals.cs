...
namespace RivetReach
{
    ...
    {
        public static bool UsesModel(byte id)=>FishId.Added(id)||id>=220&&id<=235||id==BlockId.Potato||id==BlockId.BakedPotato||id==BlockId.Apple;
        public static string Key(byte id)=>(FishId.Added(id)||id>=220&&id<=235)?id.ToString():id==BlockId.Apple?"Apple":id==BlockId.BakedPotato?"BakedPotato":"Potato";
        public static Texture2D Icon(byte id)=>Resources.Load<Texture2D>(Folder(id)+Key(id)+"Icon");
        static string Folder(byte id)=>FishId.Added(id)?"Fishing/":id>=220&&id<=235?"Farming/":id==BlockId.Apple?"Orchard/":"Food/";
        ...
        public static GameObject Create(byte id,Transform parent,Material heldMaterial=null)
        {
            ...
            {
                renderer.sharedMaterial=heldMaterial!=null?heldMaterial:Resources.Load<Material>(FishId.Added(id)?"Fishing/Fishing":id>=220&&id<=235?"Farming/Farm":id==BlockId.Apple?"Orchard/Apple":"Food/Potato");
