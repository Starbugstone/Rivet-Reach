...
namespace RivetReach
{
    public sealed partial class RuntimeVerification
    {
        IEnumerator ReviewPlacementItems()
        {
            ...
            game.Inventory.Add(3,30000);game.Selected=0;
            ...
            Check(merged.Length==2&&merged.All(p=>p.Stack.Count==100)&&merged.Select(p=>p.Stack.Id).Distinct().Count()==2,"Two hundred drops consolidate into one stack per matching item type");
            ...
            Check(dense.Count(p=>p.View!=null)==2,"Merging removes 198 redundant item GameObjects and leaves two views");
            Check(merged.Single(p=>p.Stack.Id==2).Id==dense[0].Id&&Math.Abs(merged.Single(p=>p.Stack.Id==2).Age-200.5f)<.001f,"Stable pile identity survives and merging retains the oldest elapsed lifetime");
            ...
            items.Piles[items.Piles.Count-1].Sleeping=true;items.Step(.5f);
            var overflow=items.Piles.Except(existing).ToArray();
            Check(overflow.Where(p=>p.Stack.Id==2).Select(p=>p.Stack.Count).OrderBy(n=>n).SequenceEqual(new[]{50,500})&&overflow.Single(p=>p.Stack.Id==1).Stack.Count==100,"Merging respects the 500-item limit, leaves the exact remainder and never mixes identities");
