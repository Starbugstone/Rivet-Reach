...
namespace RivetReach
{
    ...
    public static class BatteryPower
    {
        ...
        public static long Amount(MachineState m){var cells=Cells(m);long sum=0;for(int i=0;i<cells.Count;i++)sum+=cells[i].Amount;return sum;}
        ...
        public static int Available(MachineState m,bool charge)
        {
            ...
            var cells=Cells(m);int watts=0;for(int i=0;i<cells.Count;i++){var cell=cells[i];watts+=(int)((charge?cell.Capacity-cell.Amount:cell.Amount)/BatteryStorage.MillijoulesPerWattTick);}return watts;
        }
