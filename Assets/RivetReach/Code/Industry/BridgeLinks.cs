...
namespace RivetReach
{
    public sealed partial class IndustrySimulation
    {
        ...
        Dictionary<MachineState,MachineState> bridgePartners=new Dictionary<MachineState,MachineState>();
        ...
        public string BridgeStatus(MachineState m)
        {
            ...
            return !world.Ready(m.Position)||!world.Ready(partner.Position)?"Partner asleep · load both chunks":!IsSimulating(m)||!IsSimulating(partner)?"Connecting networks…":"Linked · "+partner.Position;
        }
        public bool ConfigureLoader(MachineState m,string actor,bool enabled)
        {
            ...
            m.LoaderEnabled=enabled;Invalidate(m.Position);return true;
        }
