...
namespace RivetReach
{
    ...
    public sealed class SaveStore
    {
        ...
        public SaveStore(string path,ItemRegistry registry)
        {
            ...
            content=Fingerprint(0,true,true,true,true,true,true,true,true,true,true,true,true,true,true,true,true,true,true);
            ...
            modernContent.Add(Fingerprint(0,true,true,true,true,true,true,true,true,true,true,true,true));
            modernContent.Add(Fingerprint(0,true,true,true,true,true,true,true,true,true,true,true));
        ...
        static string ItemFingerprint(ItemDefinition definition,bool farming,bool materialTags,bool compost)
        {
            string json=JsonUtility.ToJson(definition);
            if(!compost&&CompostId.OriginalInput(definition.runtimeId))
            {var old=JsonUtility.FromJson<ItemDefinition>(json);old.tags=old.tags.Where(tag=>tag!="compostable").ToArray();json=JsonUtility.ToJson(old);}
            if(farming)
            {
        ...
        static string MobFingerprint(MobDefinition definition,bool floater,bool habitats,bool spawnLight,bool playtest)
        {
            string json=JsonUtility.ToJson(definition);
            bool originalSupport=definition.spawnRules.OriginalSupport;
            if(!playtest&&definition.spawnRules.AlphaSupport)
            {
                // Project only the exact two appended buildable floors. A changed or
                // reordered original whitelist remains a real content mismatch.
                string full="\"supportBlocks\":["+string.Join(",",definition.spawnRules.supportBlocks.Select(id=>"\""+id+"\""))+"]";
                string previous="\"supportBlocks\":["+string.Join(",",definition.spawnRules.supportBlocks.Take(definition.spawnRules.supportBlocks.Length-2).Select(id=>"\""+id+"\""))+"]";
                json=json.Replace(full,previous);originalSupport=true;
            }
            if(!spawnLight)
            {
                // Only the approved hostile range projects to definitions from before light rules.
                // Keep unrelated profile, combat, item and recipe edits protected.
                if(definition.spawnRules.minimumLight!=0||definition.spawnRules.maximumLight!=7)
                    return json+"|unrecognized-spawn-light-migration";
                json=json.Replace(",\"minimumLight\":0,\"maximumLight\":7","");
            }
            ...
            if(!originalSupport||definition.spawnRules.habitat!=(isFloater?MobHabitat.Underground:MobHabitat.Surface)||isFloater&&definition.nocturnal)
