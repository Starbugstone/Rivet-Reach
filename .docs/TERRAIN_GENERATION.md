...
The user requested a terrain rework with caves, varied landscapes and biomes. The current working profile is generator **`terrain-4-biomes-caves`**, in the existing `surface` world. It replaces the previous height/cave algorithm while preserving the [finite ore profile and bedrock base](ECONOMY.md#current-ore-generation-and-bedrock), integer coordinates, chunk streaming and authoritative session edits. Numerical shapes, frequencies and material choices below are implementation defaults for review, not user-selected tuning or proven gameplay quality.
