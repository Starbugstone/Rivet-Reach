...
`DEVELOPMENT_STRATEGY.md` owns milestone scope and sequence. `PROJECT_PLAN.md` mirrors it; specialist files define behaviour. The authorized playable increment includes terrain/FPS/inventory, modular personal and workbench crafting, tool tiers, stations and survival, day/night, caves/biomes and native mobs. [Current run instructions](FIRST_POC.md) and [verification](verification/README.md) distinguish implemented behavior from measured evidence and remaining review. Review that result before selecting further implementation. Later roadmap groups remain provisional candidates and may be split or reordered; they are not an automatic implementation queue.
