...
Place a [Chest](Item-chest.md), [Bulk Crate](Item-bulk-crate.md) or [Crate Controller](Item-crate-controller.md) on its left. Attach [Item Pipe](Item-item-pipe.md) on any free face, with a red Output arrow set using a selected [Wrench](Item-wrench.md). It transfers up to four items per second and supports optional signal control. Direct chest-to-pipe output is also available; the Extractor adds signal control. See [Pipes](Pipes.md).
