...
- Lethal damage drops carried inventory, personal-grid ingredients and equipped armor exactly once, shows a death screen and offers Respawn. World construction, station contents, crops and edits remain. Respawn restores health/food and resets movement. **User revision, 2026-09-11:** the default destination is dry supported ground within a horizontal 100-block radius of world X/Z **0:0**, independent of death position, chunk unloading or the floating render origin. Search outward from the origin, respecting existing construction and fluids, with two cells of dry headroom; a flooded or blocked surface rejects that column rather than selecting a cave below it. Never fall back outside that circle. Destination terrain (including support) must load before simulation resumes and the two-second damage-immunity period begins. If the whole circle has no safe space, remain on the death screen with an explanation rather than spawning inside a hazard or destroying construction. A saved bed spawn will override this default when beds are implemented; this change does not add beds or a saved spawn binding. `Expedition.TakeDamage` is the common health authority for creature attacks and other damage sources; `Respawned` lets creature behavior clear stale pursuit independently. Drops retain the existing pickup/despawn rules.

These controls and values form the current playable survival slice. Tool durability, enchantments, irrigation, breeding and additional armor customization are not represented as implemented. [Durable saves](SAVES.md) preserve survival state, including hunger, armor and active healing. [Survival verification](verification/SURVIVAL_RESULTS.md) records checks and outstanding review.
