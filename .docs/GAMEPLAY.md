...
Following the additional user request to make the review props selectable, a fresh session provides one **Starter dagger in slot 10**, one **Starter pickaxe in slot 11**, and one **Starter axe in slot 12** so the mechanic is immediately playable; personal crafting was subsequently activated in section 16. Use the mouse wheel, `[ / ]`, or inventory to select these items. Each has a one-item stack limit. The axe has a textured original model and the existing authored one-hand tool grip. The user subsequently requested Minecraft-inspired positioning: the hand/axe assembly sits lower and farther right with a diagonal tilt, leaving the aim area clear. The blade faces forward toward what is being chopped, including during the swing. The dagger uses the existing one-hand grip and the pickaxe the two-hand grip. Both use ordinary single-block mining; neither gains axe felling. The starter items have no durability; the subsequently authored recipes are in [ECONOMY.md](ECONOMY.md#current-playable-starter-recipes), and dagger combat is not implemented. Working log break time is 1.5 seconds with fists/other items and 0.45 seconds with axe capability; leaf break time is 0.2 seconds. These are implementation defaults, not measured feel or a selected tool-progression economy. The axe is an item and cannot be placed as a terrain voxel.
...
Explore and dig to discover coal, copper, iron, gold and diamond veins. Select the starter pickaxe in hotbar slot 11 using the wheel or `[` / `]`, then hold Mine on ore within the existing 5 m reach. Bare hands, axe and dagger cannot remove ore or build mining progress; the target label says **Requires a pickaxe**. Pickaxe capability is checked again by the world mining command. Changing tools or targets resets accumulated work as before. Current mining times are 1.1 seconds for coal, 1.3 for copper, 1.5 for iron, 1.8 for gold and 2.1 for diamond, all working defaults. No tool grades or durability are introduced in this step.
