...
One voxel edge represents one metre. Start with a 0.6 m wide, 1.8 m tall player collision volume, 4.5 m/s walk, 6.5 m/s sprint and a jump reaching 1.6 blocks, explicitly chosen by the user for future half-block clearance. Half blocks themselves remain later work. Sprint starts with Left Ctrl by default or a double tap of the mapped Forward action; Left Shift crouches. The double-tap gesture has an initial 0.3-second press-to-press window and stays active while forward is held; releasing forward, crouching, inspecting or opening a menu/inventory clears it. Sprint has no stamina meter in the initial survival rules. Crouch reduces height/speed and prevents stepping off an edge unless the player deliberately jumps. Water slows movement; swimming uses held vertical input. These values need feel testing rather than physical realism.
...
**Placement facing correction — 2026-09-12:** newly placed furnaces, chests, workbenches and industrial machines turn their authored front (local −Z) toward the player, snapped to the nearest of four horizontal directions from the cell center. Height and camera pitch never tilt a machine; directly above/below the center falls back to player yaw. Models rotate about the cell center. A hand crank attached to a battery retains its rear-socket attachment rule. Facing persists through presentation residency and [save/load](SAVES.md#station-facing-compatibility--2026-09-12); older starter stations keep their original orientation. [Facing verification](verification/PLACEMENT_FACING_RESULTS.md) records the focused checks.

The player has **15 hotbar slots plus 56 backpack slots**, arranged as seven rows of eight (expanded by user request on 2026-09-12). Direct keys 1–0 select the first ten slots; mouse wheel and rebindable next/previous actions cycle through all fifteen. Older saves keep their hotbar assignments and backpack positions, with the added slots empty; [save compatibility](SAVES.md#expanded-player-inventory--2026-09-12) owns the migration. The selected survival content uses 64-item ordinary stacks and one-item tool/armor stacks. Larger industrial stack tuning remains a future decision. These are content definitions, not hard-coded assumptions in inventory storage.

Tab (rebindable Inventory) toggles inventory. E (rebindable Interact) also closes an open inventory, including station and machine screens, through the same ingredient/cursor return path. Escape closes these screens directly to play, including while a search field is focused or Pause has been rebound. It consumes the press before other bound actions, so the same key cannot reopen the interface or pause the game. Active control rebinding retains priority. Inventory and Interact shortcuts are ignored while editing a text field. A single Interact press cannot both close inventory and reopen the targeted station, or immediately close a station it just opened.

Support quick transfer, splitting, combining compatible stacks, drag placement and a full-inventory message. Pickup accepts as much of a pile as fits and leaves the remainder with its identity/timer. Normal drop releases one item; modified drop releases the stack. A 0.75-second owner pickup delay prevents instantly recollecting an intentional throw; another player may collect it under ordinary pickup rules. Automatic pickup radius is 2.04 blocks (2.04 m), a 20% increase over the previous 1.7 m. World-action drops, including mined blocks, harvested crops and axe-felled logs, receive 120% of that normal radius (2.448 m) on those exact piles. This bonus lasts while the pile remains in the world, survives partial pickup and displacement, and does not carry through inventory into a later manual throw. Piles with different pickup eligibility do not merge, even when their item type matches. Both radii require no solid barrier between player and pile. Passable placed blocks (including item/fluid pipes and cables) permit ordinary collection through them and from within their cells; open doors also permit collection, while closed doors and solid machines block it. Eligible players compete in a stable tick/entity order; a pickup transfers only actual accepted quantity. The delay/ownership metadata participates in legal merge compatibility.

### Machine illustrations — 2026-09-12

Opening a machine or starter station replaces the inventory's left character portrait with a framed illustration and its registered item name. The authored inventory artwork identifies the opened machine type; current operation, input/output, power and storage remain in the existing controls on the right. This includes furnaces, chests, both crafting benches, batteries and individual multiblock fittings. Personal inventory restores the player portrait, and equipment/appearance controls remain accessible.

The retained inventory shell rebinds this card on every station open. It reuses the existing texture without an additional camera, model, or simulation state. The image is static identification art, not a live gauge or a preview of the player's assembled multiblock. [The player guide](wiki/Machine-interfaces.md) shows the layout; [verification](verification/MACHINE_INTERFACE_RESULTS.md) records the dated native captures and limits.
...
The [lava extension](FLUIDS.md#lava--2026-09-13) adds source-only bucket transfers, slow nonrenewing flow, near-bedrock lakes in new worlds, dropped-stack destruction and rapid heat damage with lingering fire. Water extinguishes fire; Creative is immune. Its exact flow, damage and save rules belong to `FLUIDS.md`.
...
**Dropped-stack readability, 2026-09-11:** a single dropped item shows one item model; two items show two overlapping copies, and three or more show three. The compact stepped silhouette retains the item's existing art and updates after merging, partial pickup and view recreation (including loaded saves). Copies are presentation only: stack quantities, collision, collection radius and merging rules remain authoritative. The three-copy cap indicates “multiple”, not an exact count. [Verification](verification/DROP_STACK_RESULTS.md) records the focused evidence and limits.
...
**User-selected extension, 2026-09-12:** destroying natural leaves, including automatic decay after chopping a trunk, can drop a sapling and/or an edible apple. Working tuning uses independent 5% sapling and 2% apple rolls per removed natural leaf. Rolls occur only after successful removal; saved world state preserves the random sequence. Manually mined leaves also retain their existing leaf-block drop. Player-placed leaves return their leaf block when mined and never produce bonus fruit or saplings, preventing repeated placement from multiplying resources. Leaves on sapling-grown trees count as natural. Drops use the ordinary physical piles, lifetime, overflow and action-pickup rules.

Use a selected sapling on dry grass or dirt to plant it. Survival consumes one on success; Creative retains the stack. Saplings have a small woody stem and leafy shoots, are targetable and do not block movement or daylight. Mining one, removing its soil, or replacing it with water returns one sapling. Unsupported, occupied or underwater placement consumes nothing.

The growth scheduler attempts a four-to-six-log broadleaf tree after 180 seconds of unpaused world simulation. This is a working balance default, not a user-selected duration. Growth needs geometric skylight and a fully resident, empty tree footprint; construction, other foliage, water, player or creature occupancy postpones the entire change. It does not partially build a tree or overwrite blocks. Unloaded/dark jobs retry on the existing plant scheduler; a ready but obstructed footprint retries after five seconds. The deadline passes while distant terrain is unloaded, but the tree only grows once its footprint is resident and clear. Pausing and time outside the game grant no growth. A bounded queue shares the existing 16-plant-change budget; no per-sapling GameObjects or world-wide scans are introduced.

Grown logs support normal axe felling. Grown provenance and sapling deadlines survive chunk streaming and durable saves. Replacing a grown log with player wood removes that provenance immediately, preserving buildings even during queued felling. One generic sapling currently grows a broadleaf tree in any biome with suitable soil; species-specific seeds are later scope.

Hold Use for the existing 1.2-second eating action to eat an apple and restore four food points (two icons). Apples stack to 64, use an original mesh for held food and drops, and share food's cancellation/full-hunger rules. See [orchard verification](verification/ORCHARD_RESULTS.md) for the tested artifact and remaining review.
...
Shift-clicking an ingredient returns what fits to inventory. **Return ingredients** and closing inventory do the same for the whole grid. Any remainder stays in that session's grid and is visible when inventory reopens; it is never deleted or dropped by ingredient return. Existing cursor-on-close behavior still returns the cursor stack and physically drops overflow. Appearance/menu transitions that close inventory also return its ingredients. UI rebuilds do not own/reset grid contents. World and inventory state, including retained crafting ingredients, now persist through [named saves](SAVES.md).

The [item sidebar and recipe browser](CRAFTING.md#item-sidebar-and-recipe-discovery--2026-09-10) reads the same catalogs and shows every current item, grid layout, material quantity and required station. Click browser icons for recipes; right-click for uses; Shift-click (or Ctrl-click) to place one recipe into the open compatible grid; Ctrl+Shift-click to fill the maximum complete batch. Hold right-click across inventory/crafting cells to deposit one item per visited slot. Hover ordinary inventory slots and use R/U to inspect without changing stack gestures. This user-selected first browser exposes all registered items; discovery filtering and pinned goals remain future extensions. The current starter set and its working balance are owned by [ECONOMY.md](ECONOMY.md#current-survival-recipes-and-tiers). Authoring, matching, conflict rules and future extension boundaries are owned by [CRAFTING.md](CRAFTING.md).
...
**Working defaults:** a complete day lasts 20 minutes of running simulation, beginning at 08:00 on Day 1. Sunrise is at 06:00 and sunset at 18:00. The sun travels east to west; the moon follows the opposite arc at night. Sky colors, clouds, ambient light and distant fog blend through dawn/day/dusk/night. Directional shadows advance on fixed sun/moon samples (one in-world minute) and hold between them to avoid constant shadow-edge crawl. Stars appear after dusk. Moonlight is strongest at full moon and absent at new moon; a low ambient floor keeps terrain readable. These durations, colors and visibility levels remain tuning choices awaiting player review.
...
- Hunger has 20 food points, shown as ten food icons with half icons for odd points. Survival also spends one food point per approximately 136.5 seconds at rest (working Alpha tuning); activity and healing spend additional exhaustion. Walking uses 0.0075 exhaustion per metre, sprinting 0.075, jumping 0.15 and successful mining/tilling 0.0375; four exhaustion spend one food point. Food at 6 or below prevents sprinting. Hold Use for 1.2 seconds to eat a held raw potato (+1), apple (+4) or baked potato (+5). Full hunger does not consume food. Planting on farmland takes precedence over eating.
...
- Lethal damage drops carried inventory, personal-grid ingredients and equipped armor exactly once, shows a death screen and offers Respawn. World construction, station contents, crops and edits remain. Respawn restores health/food and resets movement. **User revision, 2026-09-11:** the default destination is dry supported ground within a horizontal 100-block radius of world X/Z **0:0**, independent of death position, chunk unloading or the floating render origin. Search outward from the origin, respecting existing construction and fluids, with two cells of dry headroom; a flooded or blocked surface rejects that column rather than selecting a cave below it. Never fall back outside that circle. Destination terrain (including support) must load before simulation resumes and the two-second damage-immunity period begins. If the whole circle has no safe space, remain on the death screen with an explanation rather than spawning inside a hazard or destroying construction. A valid saved [bed home](BEDS.md) overrides this default; a destroyed or unsafe bed falls back to this world-spawn search with an explanatory message. `Expedition.TakeDamage` is the common health authority for creature attacks and other damage sources; `Respawned` lets creature behavior clear stale pursuit independently. Drops retain the existing pickup/despawn rules.

These controls and values form the current playable survival slice. Tool durability, enchantments, irrigation and additional armor customization remain later work; [chickens](CHICKENS.md) have persistent breeding. [Durable saves](SAVES.md) preserve survival state, including hunger, armor and active healing. [Survival verification](verification/SURVIVAL_RESULTS.md) records checks and outstanding review.
...
- Creative starts with flight off. Double-tap the bound Jump action (Space by default) within 0.3 seconds to toggle hovering flight. A single press jumps while walking; holding Jump rises while flying. Movement bindings steer horizontally, Crouch (Left Shift) descends, and Sprint (Left Ctrl or double-tap Forward) accelerates. Flight off retains Creative invincibility and unlimited placement, with normal walking/crouching/gravity. The same voxel collision, creature collision and ready-chunk boundaries apply. Working speeds are 7 blocks/second, or 12 while sprinting, with normalized diagonal movement. Inventory blocks flight input and holds position while flying; Escape pauses the world. Opening an interface clears pending double taps without changing flight state.
...
- **Tab → All Items** shows the full registered item catalog with icons, names, search and scrolling. Clicking supplies one legal full stack through the normal inventory authority. A full inventory rejects the entire grant with visible feedback. The right-hand item sidebar also supports left-button dragging into a chosen backpack or hotbar slot: an empty slot receives a full stack and a matching stack is topped up. A different item or full target rejects the drop without replacing anything. Canceled drags, Survival drags and drags while carrying a cursor stack grant nothing. Sidebar clicks retain recipe/uses behavior. **Crafting** switches back to the personal grid; placed stations retain their regular interfaces.
- Block placement, sapling planting and potato planting retain the selected stack. Buckets retain their normal fill/empty behavior. Mining is instant, including with fists, and bypasses material/tool-tier requirements. Fists and held non-tool items remove blocks without item drops, including stored contents and attached fittings; selected tools (including the wrench) retain ordinary drops and portable stored contents. Reach and protected bedrock retain their current rules. These mining changes apply only to Creative; Survival timing, tool requirements and drops are unchanged.
...
Use a selected torch against a solid floor or wall within normal five-block reach. Torches occupy a dry air cell, attach to that face, and do not block movement or skylight. Camera clearance follows movement solidity: torches, every potato growth stage and other pass-through props never lower the first-person eye or obstruct the inspection camera. They remain selectable by interaction rays. Ceiling and underwater placement reject without spending an item. Survival consumes one torch per successful placement; Creative retains its stack. Torches are targetable with the normal voxel outline and can be mined by hand to recover one item. Removing their supporting block or flowing water entering their cell also returns exactly one torch and removes the light. They stay lit without fuel expenditure or burnout. Held/dropped torches use an original item silhouette. **Held lighting extension — 2026-09-12:** selecting a nonempty torch stack also illuminates the nearby world with the same warm, shadowed fourteen-block light as a placed torch. One reusable held light follows the flame, clamped to the player’s side of solid walls. Inventory and pause retain the illumination near the player while hiding the hands; switching to another item, emptying the selected stack or returning to the title turns it off. Holding a torch spends no fuel or items. Dropped and unselected torches do not emit light.

Attachments and voxel edits survive chunk unloading, floating-origin shifts and [named save/load](SAVES.md). Cached placed light illuminates surfaces throughout loaded terrain independently of the camera's point-light pool. Torch models are batched per chunk and visible through the terrain view distance, with terrain fog. Up to eight nearby shadowed torch accents plus one held light provide additional detail; placed accents use stable slots and gradual transitions, fading with distance from 48 to 64 blocks. The shared [lighting contract](LIGHTING.md) owns the presentation and measurement limits. [Cave lighting](LIGHTING.md) now makes placed torches, powered lamps and lava usable crop/sapling/compost light sources, independently of the camera light pool. Grass retains its existing geometric skylight checks; mob spawning retains its own species rules. Torch brightness, appearance and dense-room lighting need player review. [Torch verification](verification/TORCH_RESULTS.md) records the actual build, checks and captures.
...
## Alpha playtest interactions — 2026-09-19

[Issue #12](https://github.com/Starbugstone/Rivet-Reach/issues/12) reduces all ordinary hunger expenditure by 25% through one shared tuning multiplier; healing retains six exhaustion per recovered health point. Holding Jump repeats the normal jump when grounded, with no extra airborne jump. These are working defaults requiring continued Survival playtesting.

Middle-click over a personal inventory section or chest consolidates compatible partial stacks and sorts consistently. Hotbar and backpack remain separate sections. Stored energy/liquid metadata remains part of stack compatibility; recipe grids and typed machine slots retain their positional meaning. Holding a cursor stack or painting slots prevents sorting.

World middle-click uses the current selected target. Survival first selects an owned hotbar stack, otherwise swaps an owned backpack stack into the selected slot, and does nothing if the item is absent. Creative selects an existing suitable hotbar stack or supplies a usable stack. Crops pick their planting/produce item; upper doors resolve to the door item.

Use on a ripe crop supported by tilled farmland performs its normal harvest. A selected hoe inserts that harvest directly into inventory, drops safe overflow, then consumes one actually available planting item to replant at the initial stage. Without a seed in the resulting inventory, the cell stays empty. Wild crops and unripe crops retain ordinary break harvesting.

Block definitions own cached selection shapes and optional state-dependent providers, independently of collision and rendering. Full cubes retain the voxel traversal fast path. Crops, saplings and oriented torches are selected only through their clickable geometry. Mining, Use, crop harvesting and Pick Block consume the same nearest block/entity result. Ordinary hit/break/place presentation adds no artificial light; real placed sources keep their usual lighting updates.

The ExplorerSkin rendering path uses the world's explicit distance-fog range, preserving visible skin textures when Unity's built-in fog variant is absent. Both model choices and local skins retain their existing assets and gameplay dimensions. [Alpha verification](verification/ALPHA_PLAYTEST_RESULTS.md) distinguishes code checks, actual player captures and remaining review.
