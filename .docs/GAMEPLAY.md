...
A fresh session provides one **Starter axe in hotbar slot 12** so this authorized mechanic is playable with crafting still a placeholder. Use the mouse wheel, `[ / ]`, or inventory to select it; it has a one-item stack limit, a textured original axe model and the existing authored one-hand tool grip. The user subsequently requested Minecraft-inspired positioning: the hand/axe assembly sits lower and farther right with a diagonal tilt, leaving the aim area clear. The blade faces forward toward what is being chopped, including during the swing. This initial axe has no durability or recipe. Working log break time is 1.5 seconds with fists/other items and 0.45 seconds with axe capability; leaf break time is 0.2 seconds. These are implementation defaults, not measured feel or a selected tool-progression economy. The axe is an item and cannot be placed as a terrain voxel.
