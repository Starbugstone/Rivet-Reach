...
- Hunger has 20 food points, shown as ten food icons with half icons for odd points. Survival also spends one food point per 102.4 seconds at rest (working tuning); activity and healing spend additional exhaustion. Walking uses 0.01 exhaustion per metre, sprinting 0.1, jumping 0.2 and successful mining/tilling 0.05; four exhaustion spend one food point. Food at 6 or below prevents sprinting. Hold Use for 1.2 seconds to eat a held raw potato (+1), apple (+4) or baked potato (+5). Full hunger does not consume food. Planting on farmland takes precedence over eating.
...
Attachments and voxel edits survive chunk unloading, floating-origin shifts and [named save/load](SAVES.md). The working presentation uses warm point lights with a fourteen-block range and real terrain shadows. Up to eight placed-torch lights, plus one selected held-torch light, are active at once. Placed lights are selected within 64 blocks of the observer; the nearest torches receive those pooled lights. They fade smoothly from zero at 64 blocks to full intensity by 48 blocks. Torch geometry is displayed within 64 blocks. This is a bounded first rendering implementation, not a measured final lighting budget. [Cave lighting](LIGHTING.md) now makes placed torches, powered lamps and lava usable crop/sapling/compost light sources, independently of the camera light pool. Grass retains its existing geometric skylight checks; mob spawning retains its own species rules. Torch brightness, appearance and dense-room lighting need player review. [Torch verification](verification/TORCH_RESULTS.md) records the actual build, checks and captures.
