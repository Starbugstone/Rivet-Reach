...
Use the existing grid targeting/reach and held mining progress rules. Every ordinary block in the first terrain palette must be mineable with fists; use definition-driven durations and yields. This explicitly supersedes the wooden/stone-pick prerequisites in section 9 for this slice. Beyond the starter tools authorized below, tool progression, ore progression and future special unbreakable blocks are later content, not hidden requirements for this first loop.
...
Following the additional user request to make the review props selectable, a fresh session provides one **Starter dagger in slot 10**, one **Starter pickaxe in slot 11**, and one **Starter axe in slot 12** so this authorized mechanic is playable with crafting still a placeholder. Use the mouse wheel, `[ / ]`, or inventory to select these items. Each has a one-item stack limit. The axe has a textured original model and the existing authored one-hand tool grip. The user subsequently requested Minecraft-inspired positioning: the hand/axe assembly sits lower and farther right with a diagonal tilt, leaving the aim area clear. The blade faces forward toward what is being chopped, including during the swing. The dagger uses the existing one-hand grip and the pickaxe the two-hand grip. Both use ordinary single-block mining; neither gains axe felling. The starter items have no durability or recipes, and dagger combat is not implemented. Working log break time is 1.5 seconds with fists/other items and 0.45 seconds with axe capability; leaf break time is 0.2 seconds. These are implementation defaults, not measured feel or a selected tool-progression economy. The axe is an item and cannot be placed as a terrain voxel.
