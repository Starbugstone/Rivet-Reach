...
Every cell in the candidate bounds must be ready, including the cavity. An unloaded part pauses formation and normal transfers, while controller identity, amount, fluid identity and machine/pipe configuration remain in session memory. Reload schedules validation and restores the same instance. No offline production is granted. Chunk residency notifications use the same bounded controller footprint index as edits, including missing shell and interior cells: unrelated terrain streaming does not queue those controllers. Unload notifications run after the entire released batch is unavailable. An affected structure suspends its shared automation component while bounded reconstruction replaces its channel graphs atomically; unrelated components continue running. Multiblock shared storage remains in the same conservative dependency component as its connected machinery. Reconstruction does not add chunk tickets or catch-up production; explicit Chunk Loaders remain the player's responsibility.
