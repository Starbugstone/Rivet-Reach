...
`PipeConnections.Ports`, `WorldFaces` and `Matches` provide the shared six-neighbor connection contract used by all four `NetworkTopology` graphs. Connected pipe meshes follow the matching graph's faces; item pipes also show terminating chest connections. Chests do not bridge networks. Changing rotation, channel fittings, mode, structure formation or residency rebuilds topology. No channel discovery scans terrain every render frame.
