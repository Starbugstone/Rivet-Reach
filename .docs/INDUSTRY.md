...
Item pipes deliver machine outputs, extractor contents or configured chest outputs to compatible machine inputs, furnace ingredients/fuel or a chest inlet. A chest, furnace and each individual machine face terminate a graph; they cannot invisibly bridge separate pipe runs. One source inventory advances at most four items/s across all of its output ends, with rotating source identities and equal destination shares. Source candidates are captured before transfers so a newly received item cannot be forwarded by an initially empty chest in the same phase. Fluid pipes carry water quantities, never world flow cells or particles. Each source port transfers at most 100 mL/tick, and all fluid transfers reserve against amounts at the beginning of the transfer phase. A tank cannot forward newly received water in that same phase.
...
The user requested automated crusher → furnace transfer and compatibility filtering for item/fluid networks. Furnaces now expose all six faces to Item Pipe, defaulting to blue Input; the selected wrench switches each end independently. Red Output extracts only finished results. Inputs use the compiled furnace recipe/fuel registry: the rotated rear accepts fuel only, and the other five faces accept ingredients only. Logs at the rear burn as fuel; logs at another input face become charcoal. Manual shift-transfer still prefers ingredients. No pipe can insert into the result slot or extract furnace ingredients/fuel. Full or mismatched slots retain cargo at the source. Mixed chests try other available stacks when one is rejected, within the existing shared four-items/s limit. Recipe, face and capacity checks decide whether a receiver is compatible. They do not grant a machine or chest a preferred-first turn over another compatible receiver at the same item-pipe priority; that tie uses the shared round-robin routing rule. Source snapshots and throughput budget remain shared.
...
`NetworkTopology` shares graph discovery between independent `SignalNetworkService`, `PowerNetworkService`, item and fluid channels. A relay has separate input/output vertices. Topology changes on machine/port edits or relevant chunk residency, never by scanning ordinary voxels each render frame. Cached automation components conservatively group physically adjacent machines, shared external inventories, connected warehouse cells and named bridge partners across all channels. This dependency grouping prevents two graphs from independently spending the same stored resources; it does not create transport connections between incompatible ports. Chunk indexes include face-neighbor dependencies and potential multiblock footprints. Unrelated terrain streaming leaves completed factory graphs intact.

A relevant change immediately suspends the affected component's shared activity token. Unloaded machinery retains its exact inventories, fuel, liquids, energy and partial work. The reconstruction queue coalesces changes, privately discovers the affected subset and rebuilds its four channel graphs; unrelated components continue their ordinary fixed ticks. Discovery, sorting, port initialization, graph traversal and publication preparation share **2,048 operations and a cooperative 2 ms deadline per rendered frame**, across all fixed ticks in that frame. Structure validation is limited to one already cell-budgeted scan per frame. All four channels publish together; unaffected graph objects keep their identity. Changes intersecting the in-flight component/dependency set cancel the incomplete replacement. Later disjoint residency/configuration changes suspend and queue their own components without discarding that work; publication clears only the completed batch. Components selected together still cancel together. New machine registration and explicit global invalidation conservatively restart because they can change the dependency/index collections. No half-connection publishes; the last complete graphs remain available for diagnostics. Publication itself is an atomic main-thread operation, so the cooperative deadline is not a hard frame-time guarantee.

Components intentionally err on the side of sharing a pause where machinery or inventories touch. This is a correctness boundary rather than proof of unlimited factory scale; native stress measurements own the performance evidence. Residency comes only from the player and explicitly configured Chunk Loaders. The scheduler neither loads a factory automatically nor awards downscaled, offline or reconstruction catch-up production. After a residency rebuild, resident portions reconnect normally and dormant portions remain disconnected.

Network registration is independent of electrical supply. A completed topology stays published while a replacement graph is built in private; yielding or cancelling a rebuild cannot erase its connections. Affected components pause during topology work, while unrelated components continue running. Suspended machines retain their last completed demand for diagnostics but clear transient delivered power; rebuilt machines resume only after their replacement is ready. Dormant machines clear their transient power immediately. The machine interface shows **Power network: connected / not connected** separately from the received/requested watts and operating status; an empty battery or inactive generator reports **No electrical power**, never a global “Connecting networks” override. [Power-status verification](verification/POWER_STATUS_RESULTS.md) records the focused regression.
...
**Only a selected Wrench plus mouse Use (right-click by default) on the machine-facing end can cycle a connection state.** Empty hands, another selected item, a wrench elsewhere in inventory, Interact, a pipe centre, a pipe-to-pipe connection and a power cable do not change it. Crouch + Use (Shift + right-click by default) also cycles the end through the same states. One press changes once; holding Use does not repeatedly toggle. The normal five-metre, loaded-cell and line-of-sight rules apply. Ordinary Use and Interact continue to open pipe interfaces; crouch-to-place and channel fitting remain available. The HUD explains the current end direction and wrench requirement.
...
## Shared grid allocation — 2026-09-12

The user requested item and fluid grids to follow the same connected-network and fair-sharing rules as electricity. All three use `NetworkTopology` for physical connectivity and the channel-neutral `FairAllocation<T>` for identity-based, capped equal transfers. It redistributes rejected/full/exhausted shares and rotates indivisible leftovers. The allocator retains grants across sources within a phase; multiple faces referencing one destination cannot multiply its share. Each channel supplies its own compatibility, units and transfer transaction.

- Electricity serves live demand first, then equally charges eligible storage or equally draws its shortfall. [BATTERIES.md](BATTERIES.md) owns its shared-device budgets and bank behavior.
- Item sources retain one whole item per five ticks across all ends. Sources rotate by inventory identity rather than stack count. Among receivers at the same configured item priority, compatible cargo rotates by whole item across phases. Recipe and face-specific fuel checks remain compatibility gates, not a preferred-first exception to that rotation. Full or incompatible receivers relinquish their turn.
...
## Bulk crates and physical warehouses — 2026-09-19

[Bulk crates and controllers](CRATES.md) add item-only storage endpoints without creating a wireless or duplicate inventory. A Bulk Crate holds one ordinary item type up to **16,384** units; its optional manual lock reserves an empty crate for a selected type. Filled portable batteries/tanks remain excluded because their stored contents require the existing Chest inventory authority. A nonempty crate must be emptied before mining, while a controller can be removed freely because it owns no contents.

A Crate Controller aggregates at most **64** face-connected, resident crates and requires exactly one controller in that connected bank. It rebuilds this view from physical crate state; an invalid, oversized, second-controller or unavailable-terrain bank supplies no aggregate endpoint and never bridges unloaded terrain. Controller fills use deterministic assigned-before-empty selection, and individual crates remain independent if the controller is removed.

Item-pipe receivers have editable, persisted numeric priorities from **0 through 100**. The working defaults are Machine/Furnace **50**, Crate Controller **40**, direct Bulk Crate **30**, and Chest **20**. Highest eligible priority receives first; compatible equal-priority receivers rotate fairly. Existing machine recipe and face rules remain compatibility gates, but do not defeat equal-priority rotation. A physical crate identity is shared between its direct and controller endpoints, so multiple aliases cannot multiply source throughput or feed an item back into its origin. Full, incompatible, locked, metadata-bearing or unloaded destinations leave the item at its source.

The Bulk Crate is a 3×3 Workbench recipe of eight Planks around one Chest. The controller is a 4×4 Machinist's Bench recipe of one Bulk Crate, one Machine Casing, two Gold Ingots, two Iron Ingots, two Item Pipes and one Iron Cog. [Schema 15](SAVES.md#bulk-crate-compatibility--2026-09-19) owns persistence; [the player guide](wiki/Crates-and-warehouses.md) owns setup. [Verification](verification/CRATE_RESULTS.md) records native tests, routing measurements and illustrated publication.
