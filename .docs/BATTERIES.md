...
[Renewable generators](RENEWABLES.md) use this same allocator. Charging draws only from the shared generation budget; battery discharge is available only to machine demand and never becomes charge for another battery. The established separate-grid pass still permits a battery to charge from generation on one cable grid and supply a machine on another in the same fixed step, with exact shared budgets and no hidden cable connection.
...
The user requested equal sharing on each connected grid. Surplus is divided equally among batteries with room; a deficit is drawn equally from batteries with energy. A full, empty or mode-blocked endpoint receives no share in the blocked direction; unused shares are redistributed. Whole-watt leftovers rotate between eligible batteries each tick. A formed bank counts as one connected storage endpoint, using its existing per-cell accounting. Equal transfer rates do not force different initial charge levels to become equal. Displayed input and output watts are separate actual transfers during the latest fixed step, including simultaneous charging and supply. Shared-source and shared-load allocation between grids follows deterministic topology order; equal treatment across independent grids is not promised.

## Cable-defined grids — 2026-09-12

The user requested cable-break recovery, storage of all surplus, separate generator/battery/load wiring and equal transfers among batteries on a grid. Allocation first serves directly connected loads from generation across all grids, then charges batteries with remaining generation, then supplies unmet loads from shared storage. A final surplus pass refills capacity freed by supplying another grid. This allows an empty or full battery to charge and supply in one step without exceeding capacity, duplicating generation or charging other batteries from stored energy.
