...
`ArcadePresentation` consumes committed block-break events, accepted placements and actual pickups. Mining progress drives surface cracks; the authored swing phase drives contact chips. The cyan first-person swing ribbon was removed at the user’s request on 2026-09-12; punches and held-item swings have no hand trail. Stone/soil chips, wood splinters, curled leaves, soft dust, glints and rings have six reusable particle systems capped at 728 particles, with at most eight action bursts per frame inside 24 m. Particle collision is disabled. Cosmetic overload may omit effects, never world edits or item quantities. Active particles follow floating-origin shifts and are cleared between world sessions. Pause freezes simulation and the presentation clock. Settings exposes persistent **Effect intensity**; zero suppresses optional bursts while mining progress remains readable.
...
`Tools/create_potato_assets.py` authors the original raw and baked potato meshes in `ArtSource/Food/Potatoes.blend`, exports explicit FBX files and a shared ochre/russet/cream palette under `Resources/Food`, and renders transparent 256² inventory icons from those same meshes. Raw skin has uneven lobes, recessed eyes and subtle speckling; baked skin has a pale opened top and darker toasted sides. These extend the existing warm, faceted item style without changing crop art or food identities.

`FoodVisuals` supplies the meshes for palm-held items and physical drops, and `GameUI` retains their imported icons as shared resources. Both foods share one world material; the held view uses one per-player material with the existing hand-depth convention. Food UVs avoid the tool shader’s metallic palette regions 12 and 15. Both displays retain the imported FBX axis conversion; the palm adds its yaw on top. Geometry is centred at its origin and fits the existing 14 cm held and 23 cm drop presentation scales. `FoodArtBuild.Prepare` preserves import GUIDs, remaps the shared material and checks the imported scale, normals, UVs and geometry. [Potato art verification](verification/POTATO_ART_RESULTS.md) records actual source/runtime review and remaining limits.

## Starter station models — 2026-09-12

The chest, personal-progression 3×3 Workbench and Furnace now use original Blender-authored meshes in the Machinist’s Bench material family: beveled edges, dark iron, brown wooden boards and restrained brass fittings. The chest has a shaped plank lid, straps and latch; the carpenter’s bench has trestles, a vise, mallet and square; the furnace has dressed masonry, a recessed stone arch, grate and embers. These visual details do not add ingredients, tools or gameplay capabilities. Their existing one-cell occupancy, collision, interaction, crafting, storage and processing rules remain authoritative.

`Tools/create_starter_station_assets.py` writes the editable `ArtSource/Stations/StarterStations.blend`, three explicit FBX exports and transparent inventory icons 23–25. It reads the existing industrial atlas without regenerating the industrial kit or changing player assets. Use Blender’s background Python workflow; `.blend` texture links are relative. Each authored component must have positive signed volume before beveling; source meshes are checked within `[0,1]³`, share `WorkshopAtlas` and retain the industrial export/normalization axes. The source review and geometry report live beside the editable file.

`StarterStationBuild` imports only these three models, checks their metre bounds, normals/UVs, mesh budgets and adjacent terrain visibility, then builds `Builds/StarterStations/RivetReach.exe`. Use **Rivet Reach → Build starter station graphics review**, or write `Logs/starter-stations-build-request.txt` in an already refreshed, idle Editor; the matching result file records completion. `IndustryAssets.NormalizeModels` retains generated GUIDs and uses the ember material for the named furnace part on subsequent regeneration.

`StarterStationPresentation` derives nearby views from existing survival station state; no new persisted entity or save-format field is introduced. The old cube faces are excluded from terrain meshing, while neighboring terrain remains visible through the bench’s open structure. Existing voxel collision and ray targeting still occupy the entire cell. Placed, held and dropped items all use the authored meshes; the item browser and wiki use their matching rendered icons. The furnace’s separate ember renderer uses a property block driven by `FurnaceState.BurnTicks`, without adding a per-furnace light or fuel simulation. Views are released outside 64 metres or nonresident chunks and reconstructed from authority when needed.

[Starter station verification](verification/STARTER_STATION_RESULTS.md) records actual Blender/Unity evidence and remaining visual review. The local review build is separate from the published alpha.
...
## Fitted armor and ingots — 2026-09-12

[Equipment art](EQUIPMENT_ART.md) owns the original Blender sources, male/female fit, shared tier palettes, matching held/dropped/icon meshes and binding to the existing explorer skeleton. [Verification](verification/EQUIPMENT_ART_RESULTS.md) separates source renders, imported costs and actual player review. Fitted armor is now authorized and supersedes its earlier visual deferral; equipment wear and further customization remain later scope.
