...
- [Automatic deployment run 34626529277](https://github.com/Starbugstone/Rivet-Reach/actions/runs/34626529277) passed and published wiki commit `7287847` from repository commit `9e271a7`. All 272 deployed files (pages, assets and manifest) matched the reviewed publisher output byte-for-byte. Live Chromium review loaded all 127 index icons without missing/unlinked images, opened the iron-pickaxe page from its icon, followed its recipe-section jump, and clicked an ingredient icon through to the iron-ingot page. The published 3×3 recipe grid was visually inspected.
...
## Wrench and pipe guide refresh — 2026-09-12

The current Unity export contains **132 items** and **108 crafting/processing recipes**. The generated reference includes the reusable three-iron workbench wrench, its ingredient/output links and its Tools category. [Pipes](wiki/Pipes.md) teaches all-face connections, selected-wrench right-click configuration and blue Input/red Output arrows. Power, pump, tank, signal and affected item descriptions agree with the implemented rules.

The maintained wiki has **143 Markdown pages** and passes **5,646 local link/image checks**. Chromium review of GitHub-rendered Markdown loaded all 132 index icons and all seven wrench-page images, followed the iron ingredient to its item page, and checked the Pipes guide at 390×844 without document overflow. [Connection verification](verification/CONNECTION_RESULTS.md) records the game/build evidence and retained preview screenshots. The previous 2026-09-11 deployment above retains its dated identity.

## Crop icon and catalog maintenance — 2026-09-19

Item categories must describe their use; never classify every later runtime ID as a tank/battery component. The index includes every registered item once in a category and links recent additions near the top. Potato stage icons are baked from the actual world mesh by `CropIconBuild.Bake`; the existing Editor accepts the `wiki-crop-icons` build request to bake and export them. All four stages share framing so their relative growth is visible. [Catalog audit](verification/WIKI_CATALOG_RESULTS.md) records the correction and live evidence.
