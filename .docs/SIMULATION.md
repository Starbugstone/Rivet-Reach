...
Use spatial buckets keyed by item identity and bounded local merge queries every 0.5 seconds initially. Different types can share a cell and never combine. The first POC reuses the bucket dictionary/lists, clears retained pile references after each pass, and destroys the records and views emptied by merging; a single incoming pile can fill multiple eligible partial stacks in stable entity order during one pass. A candidate is within 1 m, has compatible item/metadata and ownership/pickup restrictions, has compatible movement state, and is not separated by solid terrain. Merging cannot pull a pile through a wall or change its water side. Cap a world pile at the item's normal inventory stack limit initially; merge partially and leave a remainder when needed. This preserves the shared ItemStack model and prevents an oversized pile from breaking inventory assumptions.
...
The user’s feedback extends Stage 0 with terrain-block placement. Its interaction contract is owned by [GAMEPLAY.md](GAMEPLAY.md#first-step-terrain-placement--user-feedback-extension). The runtime uses one occupancy-change path for mine/place operations: compare expected cell value, store the replacement in the session edit map, update resident neighbour halos and revisions, and immediately publish the bounded local remesh. Placement consumes the selected item only after the occupancy change succeeds. Player overlap uses the same cell-address calculation and 1 mm collision skin as swept movement. A committed edit wakes nearby loose stacks and resolves their solid overlaps before returning: prefer a clear top face, then the nearest clear horizontal face, then below, checking up to eight face distances (48 candidates) against authoritative ready terrain and complete item bounds. Completely sealed or unready regions retain the pile and retry after 0.25 eligible seconds or another terrain edit; placement does not delete drops or reset their timers. Item collision bounds are 0.33 m wide by 0.23 m high, enclosing the yawed 0.23 m display cube. This is a single local authority turn; it does not establish a network transaction or durable journal.
...
The current sky query returns 15 above the highest opaque voxel in a column and 0 below it. Start with deterministic terrain height (including generated logs after the tree extension), include edited opaque blocks above it and search down through mined holes. Cache only queried columns near the active neighbourhood; invalidate when an edit changes opacity. Session edits remain authoritative even if a roof's chunk is unloaded. This deliberately does not approximate indirect/torch light from visual ambient fill. A later light service can replace the query while keeping the tick and edit contracts.
...
## 14. Tree generation and event-driven felling

[GAMEPLAY.md](GAMEPLAY.md#trees-and-axe-felling--user-feedback-extension) owns player-visible tree and axe rules. Generator `terrain-2-trees` adds deterministic tree candidates on an eight-cell grid, with seeded offsets/presence and a local slope check. A chunk stamps candidates intersecting its interior or halo, including candidates rooted in adjacent chunks. Point queries use the same shape contract. The generator seed and output contract are immutable and worker-safe. A fixed 256-entry cache per worker/main thread reuses pure tree-candidate results, keyed by seed and grid coordinates; entries overwrite on collision, so it cannot grow with exploration. This avoids repeatedly evaluating a candidate slope and height for every unloaded leaf query. The streaming demand includes the layer above surface chunks so treetops can render across vertical seams. No object is allocated per tree or voxel.

Tool capability is read from the selected item definition. Only a successful generated-log mine with the Axe flag starts an upward connected-log operation; non-axe removal and all player-placed logs stay single-block. There is no per-tree structure object or persistent tree ID: provenance is per voxel and derived from the existing sparse edit overlay. Future save/diff compaction must retain a player-placement entry even if its block ID matches generated terrain, or store that provenance explicitly; otherwise a replaced log could incorrectly become natural again. Generated provenance is captured before the initial mutation and checked again for each queued log. A current log with any edit-overlay entry is player-modified and cannot join the felling operation, even at a formerly generated position. The queued operation permits only positions at/above the cut and follows face-adjacent generated logs; placed wood is a boundary. It applies compare-and-replace changes to the same session edit overlay even for nonresident upper chunks; only successful changes emit physical log drops. The felling operation holds the initiating capability, so switching items after the committed cut does not cancel already-earned work. Mining progress before the cut resets on item changes.

Tree work steps every 0.1 seconds of running simulation, with at most 32 queued log candidates and 8 due leaf candidates per step, one step per frame. Resident chunks touched by derived edits are rebuilt once per tree step, with normal revision checks rejecting stale asynchronous results. Unloaded edits require no mesh. Leaf support searches reuse their queue/set storage, and automatic decay does not reschedule foliage already affected by the original support removal. Natural leaves near manually removed logs/leaves are scheduled with a minimum two-second delay and retain support through up to four face-connected cells; placed foliage is identified from the edit overlay and never automatically decays. Generated and edited data remain session-only. The queue advances with active play/inventory and pauses with menus; it is finite pending work, not whole-world background vegetation ticking.

Leaves are solid for collision and targeting. For the current grass skylight approximation, leaves transmit daylight while logs block it; this is separate from their opaque rendered texture and shadow. Full leaf light attenuation, cutout leaf rendering, growth and persistent save migration are not implemented. See [tree results](verification/TREE_RESULTS.md) for generation seams, capabilities, drop accounting, unloaded edits, measured build/runtime evidence and limitations.


## 15. Ore generation and the world base

The user explicitly selected ore generation with distinct depth bands and unbreakable bedrock after crafting. Generator version `terrain-3-ores-bedrock` preserves the existing terrain/tree algorithms and adds a stone-replacement pass. [ECONOMY.md](ECONOMY.md#current-ore-generation-and-bedrock) owns material bands and yields; [GAMEPLAY.md](GAMEPLAY.md#17-ore-mining-and-bedrock) owns player interactions.

`OreGenerator.Bands` is an immutable ordered profile. Each ore uses a separate seed salt on a 16³ candidate grid. A candidate hash determines presence, centre offset and horizontal orientation. Maximum candidate probabilities at preferred heights are coal 60%, copper 55%, iron 60%, gold 28% and diamond 16%; probability tapers linearly to 35% of that value at each band's edge. Each candidate is an ellipsoid with a solid inner core and seeded boundary chips. Only cells within the ore's inclusive Y band and occupied by ordinary generated stone can be replaced. Fixed profile order resolves different-ore overlaps identically in chunk and point queries.

Chunk generation enumerates candidates intersecting the 32³ interior and one-cell halo, stamps their bounded volumes once, then performs the existing tree pass. Point queries evaluate the same candidates and shapes against the same base terrain. Negative coordinates use mathematical floor division. Seeds, signed coordinates and the profile entirely determine ore placement; there is no discovery-order PRNG, resident ore cache, per-voxel object, ore tick or separate reserve. Worker jobs share only immutable profile data. Profile/algorithm changes require a generator-version bump; existing durable-save migration remains future work.
...
The user-authorized day/night extension uses one `WorldClock` per current expedition session, exposed through `Expedition.Sky.Clock`. It stores elapsed world days in double precision, independently of loaded chunks, floating origins, camera coordinates and operating-system time. `Expedition.Update` advances it by eligible `Time.deltaTime` only during started, unpaused play/inventory, alongside the existing world simulation. No per-voxel work, terrain regeneration or dynamic sky texture allocation is performed by clock advancement. This is bounded presentation work by construction; measured performance evidence belongs to the verification report.

`TotalDays`, `Hour`, `DayNumber`, `IsNight`, `MoonPhase`, `MoonPhaseName` and `MoonIllumination` are read-only queries. `IsNight` is true before 06:00 or from 18:00 onward. Phase uses the dawn count, avoiding a midnight change in the middle of a night; skipped days advance arithmetically without a replay loop. Invalid negative/nonfinite times and world-day values above one billion reject at the mutation boundary. `SetTime` is an explicit restoration/verification seam, not a player command or a durable save implementation. Future save/multiplayer work must serialize/replicate authoritative time instead of creating a separate rendering clock. Consumers must resolve `game.Sky.Clock` dynamically because session replacement creates a new clock.

`DayNightCycle` maps that state to a camera-independent sky shader and a single shadowed URP directional light. The light follows the sun above the horizon and moon below, changing source when their direct contribution is zero. The shadowed light direction samples authoritative time once per in-world minute (0.25° of the orbit, approximately 0.833 real seconds at the current 20-minute day). It holds the exact rotation between those samples, including during repeated `Apply` calls, and jumps directly to the current sample after restoration or a long frame. This is presentation sampling, not a change to `WorldClock`, `IsNight`, machine ticks or lunar phases. The visible sky and ambient/fog colors continue smoothly; moonlight scales with lunar illumination. The slight sky/light angular difference stays below one sample. [Sun-shadow verification](verification/SUN_SHADOW_RESULTS.md) owns the native edge and cost measurements. Terrain, decorative grass, held blocks and debris consume matching ambient shader globals. A session-local sky material is created once and released with its owner. The cycle preserves the existing view-distance fog range and does not bake environment probes each frame. `DayLengthMinutes` configures the next clock reset; [gameplay](GAMEPLAY.md#day-night-and-lunar-phases) owns current tuning defaults. Grass's existing sky-access eligibility remains a cover test; vegetation growth has not acquired a separate photoperiod rule in this extension.

## Survival station and crop state

The 2026-09-09 survival extension uses the shared inventory/recipe model and a 20 Hz local clock independent of presentation. `WorldSurvival` owns a `BlockPos`-keyed station dictionary for its single owning world, an active-furnace set and an ordered next-growth schedule. Future multi-world persistence must serialize the owning world identity alongside these coordinates. There is no GameObject per station or crop; voxel meshes present their block state.

Furnaces advance through integer recipe/fuel boundaries. Validating output capacity and consuming an ingredient occur in the same synchronous authority call. The UI cannot deposit output items or choose an authoritative recipe result. Already lit fuel burns while idle; no new fuel is ignited when blocked. Immutable processing definitions support independent stations. Active loaded furnaces run while the inventory is open; unloaded furnaces sleep with stored input, fuel, output, burn balance and partial work. There is no offline production or cross-session save.

Planted crops schedule one next-stage event, at most 16 mesh-producing growth events per advance. Unloaded/covered crops retry after one second; eligible crops advance one stage and schedule the next. This avoids a frame scan of every crop and prevents a long frame from forcing an unbounded wave of synchronous crop meshes. Mining/uprooting cancels the pending event. Removal of the supporting soil also removes the crop across residency boundaries. Growth timing currently uses geometric skylight rather than hydration or a full voxel light simulation.

A frame contributes elapsed scaled time to a retained tick remainder; at most 100 ticks are serviced per call, leaving backlog for subsequent calls. Pause/death stop the universe clock; inventory/crafting do not. Health and hunger remain player state, not appearance state. Death first closes the active inventory interface, then drains personal inventory/grid/equipment to physical drops. Stations keep their own inventories. Respawn resets player state without recreating the world and emits a lifecycle event for creature pursuit.

[Crafting transaction boundaries](CRAFTING.md#transactions-and-performance-boundaries) still apply: one local authority thread, pure stack-limit lookups, no intervening callbacks during container commits. This is not a durable transaction journal or multiplayer access-control implementation. [Survival verification](verification/SURVIVAL_RESULTS.md) separates tested conservation/timing from whole-game performance and later persistence work.
...
`TryGiveCreativeItem` requires Creative and an open inventory, resolves the actual registry definition and uses `Inventory.TryAddExact` for a full legal stack. `TryGiveCreativeItemToSlot` applies the same Creative/inventory guard and fills only the selected empty or matching slot to its registry limit; incompatible slots remain untouched. Sidebar drag previews are UI state and create no inventory item until an accepted drop. Catalog UI owns no item definitions or backing inventory arrays. `FirstPersonPlayer.Flying` is separate from Creative and resets with player motion; the Jump double-tap gesture toggles it only during active control. Creative flight uses the existing swept voxel movement and mob collision constraints. [Gameplay](GAMEPLAY.md#creative-testing-mode) owns controls and transition semantics. This local testing override adds no network permission or durable-save system.


## Placed torch state and presentation

The [user-authorized torch rules](GAMEPLAY.md#torches) store occupied cells through the ordinary session edits and maintain supporting-cell addresses in `VoxelWorld`'s chunk-indexed torch attachment map. Placement validates a ready, solid adjacent floor/wall and dry destination before committing and consuming inventory. Support edits inspect only the five possible neighboring attachments. Removal clears attachment/light state and the existing `BlockMined` event supplies the recoverable item; fluid replacement emits the torch drop once. Unloaded attachment data is retained with the session, independently of GameObjects.

`TorchPresentation` queries a fixed nearby chunk neighborhood, refreshes at 5 Hz or immediately on torch changes/origin shifts, and shares eight point-light instances between nearby torches. Geometry residency and light selection are presentation bounds; they do not delete distant edits or consume fuel. This first artificial-light implementation affects URP rendering, while `SkyLight` and existing grass/crop/mob authority retain their prior contracts. [Torch evidence](verification/TORCH_RESULTS.md) measures the scoped runtime behavior, not long-session performance.
...
The [factory isolation capture](verification/FACTORY_ISOLATION_RESULTS.md) adds optional nested CSV scopes for native terrain views, terrain/fluid mesh submission, chunk activation, machine setup/preparation/advance, power preparation/load/charge/discharge and ranged-pump preparation. These extend the counter registry without adding rows to the compact overlay. Normalize combined processing and both charge passes by factory tick count; a ranged-pump scope is per machine call. Frozen graphics-control windows are explicitly separate from live gameplay acceptance; restore quality and component states before resuming live tests.

The [factory optimization pass](verification/FACTORY_OPTIMIZATION_RESULTS.md) caches power endpoint roles and phase eligibility with the atomically published topology. Live activity, available energy, demand and shared storage remain per-tick authority; inactive-role groups retain their display resets. Ranged extraction uses an optional resident-cell read capability resolved once per simulation, preserving scan order and timing. Terrain/fluid workers provide packed vertex data and bounds for main-thread submission, with existing revision rejection and resident publication ordering.
