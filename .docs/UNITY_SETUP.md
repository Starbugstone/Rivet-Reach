...
Play should show the **Start Expedition** title menu. An all-blue Game view was traced to an Editor-only Animator initialization exception and fixed; no extra scene setup is required. After a code update, stop Play, let Unity compile, then enter Play again. The [current player/audio report](verification/HIFI_PLAYER_AND_AUDIO_RESULTS.md) records the regression check separately from standalone build verification.
...
The user subsequently authorized the first POC. Runtime code, reusable Blender/FBX player assets, original skins/terrain materials and build/verification entry points are now versioned. Unity remains pinned to the same Editor and URP versions. Built-in audio, physics and screenshot modules were enabled; the subsequent player rebuild also enables the built-in Animation module for the authored FBX clips and Playables; no engine upgrade or live MCP installation was needed. The Editor reserialized template settings in its current format during the first player build.

Use **Rivet Reach → Build Windows first POC** or [Build-Windows.ps1](../Tools/Build-Windows.ps1). The local build-request poller waits for asset refresh/compilation before building in an open Editor; batch mode uses the same validated entry point when the project is closed. Build logs and package-generated performance metadata are derived local state. [Current terrain results](verification/TERRAIN_GENERATION_RESULTS.md) and [player/audio results](verification/HIFI_PLAYER_AND_AUDIO_RESULTS.md) provide standalone gameplay evidence beyond the original initialization checks.

## Script changes during a live Play session

`PlaySessionReloadGuard` defers loading newly compiled script assemblies for the live Play interval. The session contains non-serialized inventories, terrain workers and event subscriptions that Unity cannot restore through a mid-session domain reload. Reloading these assemblies previously left the UI and industry presentation referencing missing authorities and produced a continuous NullReferenceException stream.

Stop Play to apply pending code, then start a fresh session. Normal entry/exit domain reloads remain enabled. The guard balances Unity’s [LockReloadAssemblies](https://docs.unity3d.com/6000.0/Documentation/ScriptReference/EditorApplication.LockReloadAssemblies.html) call on Play exit and Editor shutdown; it does not change global Editor preferences or restart a live world. [Performance verification](verification/PERFORMANCE_RESULTS.md) covers the explicit compile-during-Play regression.
