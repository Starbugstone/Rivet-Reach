...
| --- | --- | ---: | ---: | --- |
| Rustback beetle | Wanders and climbs walls; warns for 1.4 seconds when approached within 5 m; attacks if the player stays or strikes it | 12 | 2 HP | Dark supported surface; up to 8 |
| Dusk prowler | Detects a visible player within 16 m, pursues and bites | 18 | 3 HP | Dark supported surface at night; up to 6 |
| Floater | Hovers 0.6 m above terrain, pursues a visible player within 14 m and punches | 16 | 3 HP | Dark underground caves, day or night; up to 4 |
...
Views and thinking sleep beyond 68 m. Once streaming is ready, hostile mobs whose chunk is unavailable are removed; hostile mobs beyond 112 m are also removed and may be replenished by later spawning. This is a bounded ambient population, not persistent named creatures: mob identity/health survive floating-origin shifts and temporary sleep within the active session, and are included in durable saves. Ambient distance removal does not count as a defeat and produces no loot. Initial combat grace is ten gameplay seconds; the shared `Respawned` event resets eight seconds of mob grace in addition to survival-owned respawn immunity. A large relocation also triggers this grace as a fallback.
...
Each profile has an explicit `supportBlocks` whitelist of registered stable block IDs. Every occupied support cell beneath the collision footprint must match; the full body must fit in resident terrain. The current species retain the previous grass, dirt, stone, sand, sandstone, snow and red-clay whitelist, extended with cobblestone and planks for constructed spawn floors. Authors can narrow it to `rivet:grass`, choose other registered solid blocks, and combine it with any habitat. Empty, duplicate, unknown or nonsolid entries are invalid. The shared checks do not assume whether a creature is hostile or passive.

Surface sites need overhead sky access. Underground sites require the full body below the terrain surface associated with the chunk’s generation history and opaque cover overhead; open shafts, above-ground roofs and liquid-filled hover/body space do not qualify. Both accepts either type. Underground/Both searches include heights near the observer rather than imposing the old surface-only band. Timing is independent: the existing `nocturnal` setting is checked separately by the hostile population controller. Distance, visibility, population limits and lifecycle likewise belong to the caller.
...
Working defaults chosen for this increment: 16 health, 3 HP punches, 2.4 m/s pursuit, 14 m detection, 26 m home leash, 2.1 m attack range, 0.8 s wind-up and 1.3 s recovery. The user subsequently requires **cave-dwelling Floaters that spawn only underground**. Ambient spawning now works day or night in sufficiently dark, covered underground cavities, retaining the existing spawn ring/sanctuary/visibility checks, up to four Floaters and the unchanged total cap of 14. The entire hovering body must be below the original terrain surface for that chunk’s recorded generator and have opaque cover overhead. Open shafts and above-ground roofs do not qualify; flooded/lava-filled body or hover spaces are rejected. Player-mined underground chambers can qualify. A bounded search probes at most 97 support heights per candidate column within 48 vertical metres of the player, using only resident terrain and rotating its starting height. Surface beetles/prowlers also require sufficiently dark ground. These values need balance playtesting; they are not individually user-selected numbers.
...
`MobDefinition.spawnRules` authors the shared habitat, support-block whitelist and light range. Floater uses Underground and `nocturnal=false`; beetles/prowlers keep Surface and their existing timing. Save content fingerprints include these rules. Exact compatibility projects the known default support lists and Floater habitat/timing migration back to historical definitions, while still rejecting unrelated content changes. Existing saved Floaters keep their positions and can move normally; cave-only eligibility governs new natural spawns. Explicit authored encounter/verification placement still bypasses natural habitat, light, timing, distance and visibility rules while checking support, clearance and population. No terrain is regenerated or retrofitted.
...
## Alpha spawning audit — 2026-09-19

Underground candidates sample the complete horizontal disk up to 48 m, then enforce the shared 24–48 m **three-dimensional** distance. The previous horizontal 24 m minimum could wrongly exclude caves mostly above/below the player despite sufficient actual distance. The 16 m horizontal sanctuary around the original world spawn, visibility rejection, two-second cadence, eight candidate columns and bounded vertical search remain. Cobblestone and plank floors now qualify for the three current hostile species; floor eligibility is still authored per profile.

`CheckSpawnSite` reports explicit rejection reasons for residency, support, clearance, habitat, light, player overlap and occupied hostile/passive bodies. Natural scheduling additionally records distance, sanctuary, visibility, timing and population rejection counters. Unknown/pending lighting never grants a dark spawn. The generic [cage spawner](SPAWNERS.md) calls this same site contract while owning its separate activation, cooldown and origin-based population budget. Its visual miniature is not a living mob.

Natural caps remain 14 total, with beetle/prowler/Floater limits of 8/6/4. Spawner-origin mobs do not consume those budgets; each cage owns at most five live mobs. Both hostile origins use the same distance and chunk-unload removal. Passive chickens retain their independent persistent lifecycle. [Alpha verification](verification/ALPHA_PLAYTEST_RESULTS.md) records the normal-cadence dark-room and lighting checks separately from explicit fixture placement.
