...
Each profile has an explicit `supportBlocks` whitelist of registered stable block IDs. Every occupied support cell beneath the collision footprint must match; the full body must fit in resident terrain. The current species retain the previous grass, dirt, stone, sand, sandstone, snow and red-clay whitelist. Authors can narrow it to `rivet:grass`, choose other registered solid blocks, and combine it with any habitat. Empty, duplicate, unknown or nonsolid entries are invalid. The shared checks do not assume whether a creature is hostile or passive.
