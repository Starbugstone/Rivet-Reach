...
| Floater | Hovers 0.6 m above terrain, pursues a visible player within 14 m and punches | 16 | 3 HP | Underground caves, day or night; up to 4 |
...
Working defaults chosen for this increment: 16 health, 3 HP punches, 2.4 m/s pursuit, 14 m detection, 26 m home leash, 2.1 m attack range, 0.8 s wind-up and 1.3 s recovery. The user subsequently requires **cave-dwelling Floaters that spawn only underground**. Ambient spawning now works day or night in covered underground cavities, retaining the existing spawn ring/sanctuary/visibility checks, up to four Floaters and the unchanged total cap of 14. The entire hovering body must be below the original terrain surface for that chunk’s recorded generator and have opaque cover overhead. Open shafts and above-ground roofs do not qualify; flooded/lava-filled body or hover spaces are rejected. Player-mined underground chambers can qualify. A bounded search probes at most 97 support heights per candidate column within 48 vertical metres of the player, using only resident terrain and rotating its starting height. Surface beetle/prowler rules remain unchanged. These values need balance playtesting; they are not individually user-selected numbers.
...
`MobDefinition.spawnRules` authors the shared habitat and support-block whitelist. Floater uses Underground and `nocturnal=false`; beetles/prowlers keep Surface and their existing timing. Save content fingerprints include these rules. Exact compatibility projects the known default support lists and Floater habitat/timing migration back to historical definitions, while still rejecting unrelated content changes. Existing saved Floaters keep their positions and can move normally; cave-only eligibility governs new natural spawns. Explicit authored encounter/verification placement still bypasses natural habitat, timing, distance and visibility rules while checking support, clearance and population. No terrain is regenerated or retrofitted.
