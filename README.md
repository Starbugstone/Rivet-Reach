...
Open this repository folder in Unity Hub with the pinned Editor version. Open `Assets/RivetReach/Scenes/Main.unity` and press Play. The Windows executable is generated at `Builds/FirstPOC-v2/RivetReach.exe`. See [POC controls and build instructions](.docs/FIRST_POC.md) and [measured verification results](.docs/verification/VISUAL_REVISION_RESULTS.md). See [Unity setup](.docs/UNITY_SETUP.md) for the exact setup and verification notes.
