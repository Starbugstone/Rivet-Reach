...
Open this repository folder in Unity Hub with the pinned Editor version. Open `Assets/RivetReach/Scenes/Main.unity` and press Play. The Windows executable is generated at `Builds/PlayerRevision4/RivetReach.exe`. See [POC controls and build instructions](.docs/FIRST_POC.md), the latest [body and scene polish](.docs/verification/BODY_AND_SCENE_POLISH_RESULTS.md), and the [player asset results](.docs/verification/PLAYER_POLISH_RESULTS.md). See [Unity setup](.docs/UNITY_SETUP.md) for the exact setup and verification notes.
